...
export class FileList extends data.Component<ISettingsProps, FileListState> {
        ...
        return pkg.sortedFiles().map(file => {
            ...
            return (
                <a key={file.getName() }
                    ...
                    {deleteFiles && /\.blocks$/i.test(file.getName()) ? <sui.Button class="primary label" icon="trash" title={lf("Delete file {0}", file.name)} onClick={(e) => this.removeFile(e, file) } /> : ''}
                </a>);
        })
    }
    ...
    private packageOf(p: pkg.EditorPackage) {
        ...
        return [<div key={"hd-" + p.getPkgId() } className="header link item" role="treeitem" aria-expanded={expands[p.getPkgId()]} aria-label={lf("{0}, {1}", p.getPkgId(), expands[p.getPkgId()] ? lf("expanded") : lf("collapsed"))} onClick={() => this.togglePkg(p) } tabIndex={0} onKeyPress={sui.fireClickOnEnter}>
            <i className={`chevron ${expands[p.getPkgId()] ? "down" : "right"} icon`}></i>
            {upd ? <sui.Button class="primary label" icon="refresh" title={lf("Refresh package {0}", p.getPkgId())} onClick={(e) => this.updatePkg(e, p) } /> : ''}
            {del ? <sui.Button class="primary label" icon="trash" title={lf("Delete package {0}", p.getPkgId())} onClick={(e) => this.removePkg(e, p) } /> : ''}
            {p.getPkgId() }
        </div>
...
`);
    ...
    renderCore() {
        ...
        return <div role="tree" className={`ui tiny vertical ${targetTheme.invertedMenu ? `inverted` : ''} menu filemenu landscape only`}>
            <div role="treeitem" aria-expanded={show} aria-label={lf("File explorer toolbar")} key="projectheader" className="link item" onClick={() => this.toggleVisibility() } tabIndex={0} onKeyPress={sui.fireClickOnEnter}>
                {lf("Explorer") }
                <i className={`chevron ${show ? "down" : "right"} icon`}></i>
                {plus ? <sui.Button class="primary label" icon="plus" title={lf("Add custom blocks?")} onClick={(e) => this.addCustomBlocksFile() } /> : undefined }
            </div>

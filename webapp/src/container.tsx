    ...
    render() {
        ...
        return <sui.DropdownMenuItem icon="help circle large" class="help-dropdown-menuitem" textClass={"landscape only"} title={lf("Reference, lessons, ...") }>
            {targetTheme.docMenu.map(m =>
                ...
                : !m.tutorial ? <sui.Item key={"docsmenu" + m.path} role="menuitem" aria-label={m.name} text={m.name} class="" onClick={() => this.openDocs(m.path) } tabIndex={-1}/>
                : <sui.Item key={"docsmenututorial" + m.path} role="menuitem" aria-label={m.name} text={m.name} class="" onClick={() => this.openTutorial(m.path) } tabIndex={-1}/>
            ) }

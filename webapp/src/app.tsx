    ...
    render() {
        ...
        return <div id="docsmenu" className="ui buttons">
            <sui.DropdownMenu class="floating icon button" icon="help">
                {targetTheme.docMenu.map(m => <a href={m.path} target="docs" key={"docsmenu" + m.path} className="ui item widedesktop hidden">{lf(m.name)}</a>) }
                {targetTheme.docMenu.map(m => <sui.Item key={"docsmenuwide" + m.path} class="widedesktop only" onClick={() => this.openDoc(m.path) }>{lf(m.name)}</sui.Item>) }
            </sui.DropdownMenu>
        </div>
    }
}

        ...
        return <aside className="debugtoolbar" style={{ left: xPos }} role="complementary" aria-label={lf("Debugger toolbar")}>
            {!isDebugging ? undefined :
                <div className={`ui compact borderless menu icon mini`}>
                    ...
                    {restart ? <sui.Item key='dbgrestart' className={`dbg-btn dbg-restart right`} icon={`refresh green`} title={restartTooltip} onClick={() => this.restartSimulator(true)} /> : undefined}
                    <sui.Item key='dbgstop' className={`dbg-btn dbg-stop ${!restart ? 'right' : ''}`} icon={`stop red`} title={debugTooltip} onClick={() => this.exitDebugging()} />
                </div>}
        </aside>;
    }
}
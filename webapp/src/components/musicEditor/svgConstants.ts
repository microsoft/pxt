...
export const BASS_CLEF_HEIGHT = 280;
...
export const BASS_CLEF_TOP = WORKSPACE_HEIGHT;
...
export function closestRow(y: number) {
    const isBassClef = y > BASS_CLEF_TOP
    return 12 - Math.round((y - STAFF_HEADER_HEIGHT - (isBassClef ? BASS_CLEF_TOP : 0)) / (STAFF_ROW_HEIGHT / 2))
}

export function rowY(row: number, isBassClef: boolean) {
    if (isBassClef) {
        return BASS_CLEF_TOP + STAFF_HEADER_HEIGHT + (12 - (row % 12)) * STAFF_ROW_HEIGHT / 2;
    }

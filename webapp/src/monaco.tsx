        ...
        return <div ref={e => this.rootElement = e} id='monacoEditorToolbox' className='monacoToolboxDiv'>
            <div className="blocklyTreeRoot">
                <div role="tree">
                    ...
                    {hasAdvanced ? <CategoryItem toolbox={this} treeRow={{ ns: "", category: lf("{id:category}Advanced"), color: pxt.blocks.getNamespaceColor('advanced'), icon: showAdvanced ? 'advancedexpanded' : 'advancedcollapsed' }} onCategoryClick={this.advancedClicked.bind(this) }/> : undefined}
                    {advancedCategories.map((treeRow) => (
                        <CategoryItem key={treeRow.ns} toolbox={this} selected={selectedNs == treeRow.ns} treeRow={treeRow} previousTreeRow={(treeRow as any).previousTreeRow} nextTreeRow={(treeRow as any).nextTreeRow} onCategoryClick={this.setSelection.bind(this) } />
                    )) }
                    {hasPackages && showAdvanced ? <TreeRow treeRow={{ ns: "", category: Util.lf("{id:category}Extensions"), color: '#717171', icon: "addpackage" }} onClick={this.addPackage.bind(this) } /> : undefined }
                </div>
            </div>
        </div>
    }

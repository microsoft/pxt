namespace ts.pxt.ir {
    ...
    export class UnnamedCell extends Cell {
        ...
        getName() {
            ...
            return this.owningProc.prefixForUnnammedCells
        }
    ...
    export class Procedure extends Node {
        ...
        private _prefixForUnnamedCells: string = null

        //When generating JS we need to make sure that the unnamed cells are given a name that doesn't clash
        get prefixForUnnammedCells(): string {
            if (!this._prefixForUnnamedCells) {
                let allNonTempVars = this.args.concat(this.locals).concat(this.captured).filter((cell) => !cell.isTemporary)
                let varsSharingPrefix = allNonTempVars;
                let prefix = "unnamedVar"
                let prefixSuffix = ""
                let i = 0
                let filterStartsWith = (cell: ir.Cell) => {
                    return cell.getName().lastIndexOf(prefix + prefixSuffix) == 0
                }
                while (varsSharingPrefix.filter(filterStartsWith).length > 0) {
                    prefixSuffix = "_" + i + "_"
                    i++
                }
                this._prefixForUnnamedCells = prefix + prefixSuffix
            }

            return this._prefixForUnnamedCells
        }

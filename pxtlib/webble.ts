namespace pxt.webBluetooth {
    ...
    export class PartialFlashingService extends BLEService {
        ...
        flashAsync(hex: string, token?: pxt.U.CancellationToken): Promise<void> {
            if (this.hex) {
                this.debug(`flashing already in progress`)
                return Promise.resolve();
            }
            this.device.pauseLog();
            return this.createFlashPromise(hex, token)
                .finally(() => this.device.resumeLogOnDisconnection());
        }

        private createFlashPromise(hex: string, flashToken?: pxt.U.CancellationToken): Promise<void> {
            if (this.hex) {
                this.debug(`flashing already in progress`)
                return Promise.resolve();
            }

...
namespace pxtblockly {
    ...
    export interface FieldMelodyEditorOptions {
    ...
    interface ParsedMelodyEditorOptions {
        sizes: [number, number][];
        initColor: number;
        initWidth: number;
        initHeight: number;
    ...
    export class FieldMelodyEditor extends Blockly.Field implements Blockly.FieldCustom {
        public isFieldCustom_ =true;

        private params: ParsedMelodyEditorOptions;
        private blocksInfo: pxtc.BlocksInfo;
        private editor: pxtmelody.MelodyEditor;
        private state: pxtmelody.MelodyArray;
        ...
        constructor(text: string, params: any, validator?: Function) {
            ...
            if (!this.state) {
                this.state = new pxtmelody.MelodyArray(120);
            }
        }
        ...
        //setValue(melody: string)

        getText(): string {
            let melodyString: string = "melody"; // placeholder
            return melodyString;
            //return pxtmelody.stringRepresentation(this.state);
        }
        ...
        private parseMelodyArray(newText: string) {
            // TO DO
        }
...
>>>>>>> Stashed changes
        ...
        showEditor_() {
            ...
            this.editor = new pxtmelody.MelodyEditor(this.state, this.blocksInfo, this.lightMode);
            ...
            Blockly.DropDownDiv.showPositionedByBlock(this, this.sourceBlock_, () => {
                this.state = this.editor.getMelody();
                this.redrawPreview();
        ...
        private renderPreview() {
            ...
            const cellSize = Math.min(PREVIEW_WIDTH / this.state.getWidth(), PREVIEW_WIDTH / this.state.getHeight());

            // Center the image if it isn't square
            const xOffset = Math.max(Math.floor((PREVIEW_WIDTH * (1 - (this.state.getWidth() / this.state.getHeight()))) / 2), 0);
            const yOffset = Math.max(Math.floor((PREVIEW_WIDTH * (1 - (this.state.getHeight() / this.state.getWidth()))) / 2), 0);
            ...
            for (let c = 0; c < this.state.getWidth(); c++) { // fix?
                for (let r = 0; r < this.state.getHeight(); r++) { // fix?
                    const color = this.state.getColor(r);
        ...
        function parseFieldOptions(opts: FieldMelodyEditorOptions) {
            const parsed: ParsedMelodyEditorOptions = {
                sizes: [
                    [8, 8],
                    [8, 16],
                    [16, 16],

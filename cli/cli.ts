                ...
                return pxt.crowdin.downloadTranslationsAsync(cred.branch, cred.prj, cred.key, crowdf, { translatedOnly: true, validatedOnly: true })
                    .then(data => {
                        ...
                        if (errorIds.length) {
                            ...
                            pxt.reportError("loc.errors", "invalid translation", errors as any);
                        }
...
function checkDocsAsync(parsed?: commandParser.ParsedCommand): Promise<void> {
    return internalCheckDocsAsync(
        ...
        !!parsed.flags["fix"],
        !!parsed.flags["spelling"]
    )
}

function internalCheckDocsAsync(compileSnippets?: boolean, re?: string, fix?: boolean, spelling?: boolean): Promise<void> {
    ...
    const spellIgnoreWords = [
        "JavaScript",
        "Adafruit", "Adafruit’s", "aluminium", "Annikken", "API’s", "Arduino", "assistive", "Atmel", "Atmel's", "ATSAMD", "autogenerated", "Automationbit", "AVR's", "avrdude", "backquote", "Beatbox", "BETT", "Bing", "bitwise", "bivariance", "Blinky", "Blockly", "boardname", "bootloader", "bootloaders", "Capitan", "CCIT", "Chibi", "Chibitronics", "Chromebook", "Chromebooks", "Cleverbot", "CMSIS", "Codal's", "codecard", "compostable", "CPX's", "croc", "Crowdin", "CSTA", "datasheet", "datatypes", "declareexample", "decompile", "demodulator", "desynchronized", "directionality", "Disjunction", "dmesg", "drivename", "Eddystone", "Envirobit", "eval", "firmwares", "flyout", "frowny", "fullscreen", "functionreturnexample", "GDPR", "Gigglebot", "homeurl", "hotfix", "IBDAP", "Iceweasel", "iframes", "Imagimaker", "inex", "initializers", "ints", "ISTE", "Jaqster", "Kickstarter", "Kitronik", "Kittenbot", "Ladyada", "LEDs", "localizable", "LUFA", "Magisheild", "makecode", "maxval", "mbed", "mbed's", "Mbit", "microbit", "MICROBIT", "microcontroller", "microcontrollers", "microservo", "microsoftsam", "microturtle", "milli", "MINDSTORMS", "Minecraft", "Minode", "minval", "mkcd", "mmoskal", "Moto", "multithreading", "Muselab", "neighbour", "neopixel", "Neopixels", "OLED", "Olney", "optiboot", "PDAs", "PDF’s", "pelikhan", "Pimoroni", "PPT’s", "prepend", "pseudocode", "Pseudocode", "pxtarget", "Rando", "Raspbian", "reimplementing", "repo", "Repo", "repos", "riknoll", "Robobit", "SAMD", "SBRICK", "screwable", "Scrollbit", "Seamonkey", "Seeed", "Seeed's", "Selectable", "semver", "Sidedocs", "simshim", "softdevice", "Sortees", "Sparkfun", "Squarespace", "squarespace", "substr", "subtyping", "Sunfounder", "superclass", "superset", "tballmsft", "testghpkgs", "thingiverse", "Tmega", "Toolchain", "Tooltips", "tooltips", "tronix", "typeof", "UART", "Uart", "unboxed", "undistracted", "Unmute", "unplot", "Unplot", "unshift", "Uploader", "Versioning", "Wireshark", "Wordpress", "wordpress", "Yoyo", "Zoomer"
    ];
    ...
    while (todo.length) {
        ...
        // check spelling
        if (spelling) {
            if (entrypath == '/courses') {
                console.log("Going to check spelling of " + entrypath);

                // Remove any ignores from text
                let spellCheckMd = md;
                spellIgnoreRegexp.concat(spellIgnoreWords).forEach(regex => {
                    spellCheckMd = spellCheckMd.replace(new RegExp(regex, 'gi'), '');
                });
                console.log(spellCheckMd);
            }
        }
...
function testReleaseAsync(): Promise<void> {
    let pkg = readJson("package.json")
    if (pkg["name"] == "pxt-core") {
        console.log("Run this command from a target");
        return Promise.resolve(pkg)
    }
    // Check that a local release-tests.conf.js exists.
    if (!fs.existsSync("release-tests.conf.js")) {
        console.error("This command requires release-tests.conf.js in current directory.")
        process.exit(1)
    }
    nodeutil.setTargetDir(process.cwd())
    const initialPath = process.cwd();
    // Build web app tests
    console.log("Building release tests");
    const releaseTestsPath = './node_modules/pxt-core/tests/webapp-test';
    process.chdir(releaseTestsPath);
    return nodeutil.spawnAsync({
        cmd: "node",
        args: ["../../node_modules/typescript/bin/tsc"]
    }).then(() => {
        ...
        let wdio = new Launcher('./node_modules/pxt-core/tests/webapp-test/target-browserstack.conf.js');

        return wdio.run().then(function (code: any) {
            process.exit(code);
        }, function (error: any) {
...
`)
    ...
    advancedCommand("testrelease", "run UI tests on release", testReleaseAsync);

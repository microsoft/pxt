...
import { isObjectType, getComments } from "./emitter/emitter"
...
export function simshim(prog: ts.Program, pathParse: any): pxt.Map<string> {
    ...
    */
    ...
    function promiseElementType(tp: ts.Type) {
        if (isObjectType(tp) && (tp.objectFlags & ts.ObjectFlags.Reference) && tp.symbol.name == "Promise") {

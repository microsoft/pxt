import { Diagnostic, sys, getLineAndCharacterOfPosition, DiagnosticCategory, flattenDiagnosticMessageText, Program, parseCommandLine, findConfigFile, ParsedCommandLine, parseConfigFileTextToJson, ExitStatus, parseJsonConfigFileContent, createCompilerHost, createProgram } from "../ext-typescript/lib/tsserverlibrary";
...
export function getDiagnosticString(diagnostic: pxtc.KsDiagnostic | Diagnostic): string {
    let ksDiagnostic: pxtc.KsDiagnostic;
    if (isTsDiagnostic(diagnostic)) {
        // convert ts.Diagnostic to pxtc.KsDiagnostic
...
function getDiagnosticStringHelper(diagnostic: pxtc.KsDiagnostic): string {
    let output = "";
...
function isTsDiagnostic(a: pxtc.KsDiagnostic | Diagnostic): a is Diagnostic {

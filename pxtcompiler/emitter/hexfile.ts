import { Binary, ClassInfo } from "./emitter";
import { AssemblerSnippets, ProctoAssembler } from "./backbase";
import { ThumbSnippets } from "./backthumb";
import { CompileResult } from "./driver";
...
export namespace hex {
    ...
    export function setupInlineAssembly(opts: CompileOptions) {
        asmLabels = {}
        let asmSources = opts.sourceFiles.filter(f => pxt.U.endsWith(f, ".asm"))
        asmTotalSource = ""
        let asmIdx = 0
    ...
    export function encodeVTPtr(ptr: number, opts: CompileOptions) {
        ...
        pxt.U.assert(vv < 0xffff)
        pxt.U.assert(vv << opts.target.vtableShift == ptr)
        return vv
    }
    ...
    export function setupFor(opts: CompileTarget, extInfo: ExtensionInfo, hexinfo: pxtc.HexInfo) {
        ...
        if (hex.length <= 2) {
            elfInfo = pxt.elf.parse(pxt.U.fromHex(hex[0]))
            bytecodeStartIdx = -1
        ...
        let hitEnd = () => {
            if (!bytecodeStartAddr) {
                ...
                pxt.U.assert((paddingBytes & 0xf) == 0)
            ...
            while (s.length >= step) {
                ...
                if (inf.argsFmt.length == 0) {
                    ...
                    if (!opts.runtimeIsARM && opts.nativeType == NATIVE_TYPE_THUMB && !(value & 1)) {
                        pxt.U.oops("Non-thumb addr for " + inf.name + " / " + hexb)
                    }
                }
                inf.value = value
            }
    ...
    export function validateShim(funname: string, shimName: string, attrs: pxtc.CommentAttrs,
        hasRet: boolean, argIsNumber: boolean[]) {
        if (shimName == "TD_ID" || shimName == "TD_NOOP" || shimName == "ENUM_GET")
            return
        if (pxt.U.lookup(asmLabels, shimName))
            return
        ...
        if (inf) {
            ...
            for (let i = 0; i < argIsNumber.length; ++i) {
                ...
                if (!spec)
                    pxt.U.userError("excessive parameters passed to " + nm)
            }
            if (argIsNumber.length != inf.argsFmt.length - 1)
                pxt.U.userError(`not enough arguments for ${nm} (got ${argIsNumber.length}; fmt=${inf.argsFmt.join(",")})`)
        } else {
    ...
    function patchString(bytes: ArrayLike<number>) {
        ...
        pxt.U.assert(stringVT.length == 4)
        // @PXT
        ...
        if (bytes[5] == 0x3a) {
            ...
            if (!vt) pxtc.oops("missing vt: " + isString)
            vt ^= 1
            if (vt & 3) pxtc.oops("Unaligned vt: " + vt)
            ...
            if (6 + len >= bytes.length)
                pxt.U.oops("constant string too long!")
            pxt.HF2.write16(headerBytes, 4, len)
            return headerBytes
            //console.log("patch file: @" + addr + ": " + pxt.U.toHex(patchV))
        }
        return null
    }
    ...
    export function patchHex(bin: Binary, buf: number[], shortForm: boolean, useuf2: boolean) {
        ...
        pxt.U.assert(sizeEntry < 64000, "program too large, bytes: " + buf.length * 2)
        ...
        if (elfInfo) {
            ...
            return [pxt.U.uint8ArrayToString(resbuf)]
        }
        ...
        if (uf2) {
            ...
            if (bin.checksumBlock) {
                pxt.U.oops("checksum block in HEX not implemented yet")
            }
        }
...
function emitStrings(snippets: AssemblerSnippets, bin: Binary) {
    ...
    const keys = pxt.U.unique(bin.ifaceMembers.concat(Object.keys(bin.strings)), s => s)
    for (let s of keys) {
...
export function computeHashMultiplier(nums: number[]) {
    let shift = 32
    pxt.U.assert(pxt.U.unique(nums, v => "" + v).length == nums.length, "non unique")
...
`
    ...
    s += "  .short " + pxt.U.toArray(map).map((e, i) => (offsets[e + ""] || zeroOffset) - (i * 2)).join(", ") + "\n"
    s += descs
...
`
    ...
    pxt.U.iterMap(bin.codeHelpers, (code, lbl) => {
...
function mkProcessorFile(target: CompileTarget) {
    let b: pxtc.assembler.File

    if (target.nativeType == NATIVE_TYPE_VM)
        b = new pxtc.assembler.VMFile(new vm.VmProcessor(target))
    else
        b = new pxtc.assembler.File(new thumb.ThumbProcessor())
...
function throwAssemblerErrors(b: pxtc.assembler.File) {
    if (b.errors.length > 0) {
        ...
        b.errors.forEach(e => {
            ...
            if (m) {
                ...
                userErrors += pxt.U.lf("At inline assembly:\n")
...
`
    ...
    if (res.buf) {
        if (opts.target.flashChecksumAddr) {
            ...
            pxt.U.assert(pos == res.buf.length - checksumWords * 2)
    ...
    for (let bkpt of cres.breakpoints) {
        let lbl = pxt.U.lookup(res.thumbFile.getLabels(), "__brkp_" + bkpt.id)
        if (lbl != null)
            bkpt.binAddr = lbl
    }

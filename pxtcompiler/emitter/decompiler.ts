import { Expression, Program, SourceFile, NodeArray, EnumDeclaration, isModuleDeclaration, Identifier, PrefixUnaryExpression, CallExpression, BinaryExpression, LiteralExpression, PostfixUnaryExpression, ArrowFunction, PropertyAccessExpression, SyntaxKind, VariableDeclaration, ArrayLiteralExpression, NumericLiteral, isModuleBlock, isVariableStatement, NodeFlags, isCallExpression, isIdentifier, LiteralLikeNode, ObjectBindingPattern, ParenthesizedExpression, FunctionLikeDeclaration, ModuleDeclaration, ModuleBlock, VariableStatement, isPropertyAccessExpression } from "../ext-typescript/lib/typescript";
import { LSHost } from "./languagehost";
import { ir } from "./ir";
import { pxtInfo, BinaryExpressionInfo, PxtNodeFlags, CallInfo } from "./emitter";
import { patchUpDiagnostics } from "./driver";
...
interface DecompilerEnv {
    ...
    attrs: (c: CallInfo) => pxtc.CommentAttrs;
    compInfo: (c: CallInfo) => pxt.blocks.BlockCompileInfo;
    localReporters: pxtc.PropertyDesc[][]; // A stack of groups of locally scoped argument declarations, to determine whether an argument should decompile as a reporter or a variable
...
export function buildRenameMap(p: Program, s: SourceFile, takenNames: NamesSet = {}): [RenameMap, NamesSet] {
    ...
    function collectNameCollisions(): NamesSet {
        ...
        function checkChildren(n: ts.Node): void {
...
export function decompileToBlocks(blocksInfo: pxtc.BlocksInfo, file: ts.SourceFile, options: DecompileBlocksOptions, renameMap?: RenameMap): pxtc.CompileResult {
    ...
    const checkTopNode = (topLevelNode: ts.Node) => {
    ...
    if (enumMembers.length) {
        ...
        enumMembers.forEach(e => {
            write(`<variable type="${pxt.U.htmlEscape(e.type)}">${pxt.U.htmlEscape(e.name)}</variable>`)
        });
        write("</variables>")
    }
    ...
    try {
        ...
        if ((<any>e).programTooLarge) {
            ...
            result.diagnostics = patchUpDiagnostics([{
                file,
...
${output}</xml>`;
    ...
    function error(n: ts.Node, msg?: string) {
        ...
        const diags = patchUpDiagnostics([{
            file: file,
    ...
    function attrs(callInfo: CallInfo): pxtc.CommentAttrs {
    ...
    function compInfo(callInfo: CallInfo): pxt.blocks.BlockCompileInfo {
        const blockInfo = blocksInfo.apis.byQName[callInfo.qName];
        if (blockInfo) {
    ...
    function countBlock() {
        ...
        if (emittedBlocks > MAX_BLOCKS) {
            let e = new Error(pxt.U.lf("Could not decompile because the script is too large"));
    ...
    function emitStatementNode(n: StatementNode) {
        ...
        if (n.data !== undefined) {
            write(`<data>${pxt.U.htmlEscape(n.data)}</data>`)
        }
        ...
        if (n.comment !== undefined) {
            write(`<comment pinned="false">${pxt.U.htmlEscape(n.comment)}</comment>`)
        }
    ...
    function emitFieldNode(n: FieldNode) {
        write(`<field name="${pxt.U.htmlEscape(n.name)}">${pxt.U.htmlEscape(n.value.toString())}</field>`);
    }

    function emitHandler(h: Handler) {
        write(`<statement name="${pxt.U.htmlEscape(h.name)}">`)
    ...
    function emitOutputNode(n: OutputNode, shadow = false) {
        if (n.kind === "text") {
            ...
            write(`<${tag} type="${pxt.U.htmlEscape(node.type)}">`)
    ...
    function openBlockTag(type: string) {
        countBlock();
        write(`<block type="${pxt.U.htmlEscape(type)}">`)
    }
    ...
    function emitWorkspaceComment(comment: WorkspaceComment) {
        ...
        write(`<comment h="${height}" w="${width}" data="${pxt.U.htmlEscape(comment.refId)}">`)
        write(pxt.U.htmlEscape(comment.text))
        write(`</comment>`);
    }
    ...
    function getOutputBlock(n: ts.Node): OutputNode {
        if (checkExpression(n, env)) {
            ...
            switch (n.kind) {
                ...
                default:
                    error(n, pxt.U.lf("Unsupported syntax kind for output expression block: {0}", SK[n.kind]));
                    break;
            }
            return undefined;
        }
    ...
    function getValue(name: string, contents: boolean | number | string | ts.Node, shadowType?: string, shadowMutation?: pxt.Map<string>): ValueNode {
    ...
    function getIdentifier(identifier: Identifier): ExpressionNode {
        ...
        let localReporterArg: pxtc.PropertyDesc = null;
    ...
    function getPropertyAccessExpression(n: ts.PropertyAccessExpression, asField = false, blockId?: string): OutputNode {
        ...
        if (attributes.blockId === "lists_length" || attributes.blockId === "text_length") {
            const r = mkExpr(pxt.U.htmlEscape(attributes.blockId));
            r.inputs = [getValue("VALUE", n.expression)];

            return r;
        }

        let value = pxt.U.htmlEscape(attributes.blockId || callInfo.qName);

        const [parent,] = getParent(n);
        const parentCallInfo: CallInfo = parent && pxtInfo(parent).callInfo;
        ...
        if (blockId && info.thisParameter) {
            ...
            r.inputs = [getValue(pxt.U.htmlEscape(info.thisParameter.definitionName), n.expression, info.thisParameter.shadowBlockId)];
            return r;
        }
        ...
        const r = mkExpr(pxt.U.htmlEscape(idfn.attributes.blockId));
        r.fields = [{
            kind: "field",
            name: pxt.U.htmlEscape(f[1]),
            value
        }];
        return r
    }
    ...
    function getTaggedTemplateExpression(t: ts.TaggedTemplateExpression): ExpressionNode {
        const callInfo: CallInfo = pxtInfo(t).callInfo;

        let api: pxtc.SymbolInfo;
    ...
    function getStatementBlock(n: ts.Node, next?: ts.Node[], parent?: ts.Node, asExpression = false, topLevel = false): StatementNode {
        ...
        if (err) {
            ...
            switch (node.kind) {
                ...
                default:
                    if (next) {
                        error(node, pxt.U.lf("Unsupported statement in block: {0}", SK[node.kind]))
                    }
                    else {
                        error(node, pxt.U.lf("Statement kind unsupported in blocks: {0}", SK[node.kind]))
                    }
                    return undefined;
            }
        }
        ...
        function getComments(commented: ts.Node) {
            const commentRanges = getLeadingCommentRangesOfNode(commented, file)
            if (commentRanges) {
    ...
    function getTypeScriptStatementBlock(node: ts.Node, prefix?: string, err?: string): StatementNode {
        ...
        if (err && options.includeGreyBlockMessages) {
            r.mutation["error"] = pxt.U.htmlEscape(err);
        }

        parts.forEach((p, i) => {
            r.mutation[`line${i}`] = pxt.U.htmlEscape(p);
        });
    ...
    function getImageLiteralStatement(node: ts.CallExpression, info: CallInfo) {
        ...
        if (nc * 5 != leds.length) {
            error(node, pxt.U.lf("Invalid image pattern"));
            return undefined;
        }
        let ledString = '';
        for (let r = 0; r < 5; ++r) {
    ...
    function getBinaryExpressionStatement(n: ts.BinaryExpression): StatementNode {
        ...
        switch (n.operatorToken.kind) {
            ...
            case SK.MinusEqualsToken:
                ...
                return r;
            default:
                error(n, pxt.U.lf("Unsupported operator token in statement {0}", SK[n.operatorToken.kind]));
                return undefined;
        }
    }
    ...
    function checkConditionalExpression(expr: Expression) {
        ...
        switch (unwrappedExpr.kind) {
            ...
            default:
                return pxt.U.lf("Conditions must evaluate to booleans or identifiers");
        }
        ...
        function checkBooleanBinaryExpression(n: BinaryExpression) {
            switch (n.operatorToken.kind) {
                ...
                case SK.BarBarToken:
                    return undefined;
                default:
                    return pxt.U.lf("Binary expressions in conditionals must evaluate to booleans");
            }
        }

        function checkBooleanCallExpression(n: CallExpression) {
            const callInfo: CallInfo = pxtInfo(n.expression).callInfo;
            if (callInfo) {
            ...
            return pxt.U.lf("Only functions that return booleans are allowed as conditions");
        }
    }
    ...
    function getForStatement(n: ts.ForStatement): StatementNode {
        ...
        function checkForVariableUsages(node: ts.Node): boolean {
    ...
    function getVariableSetOrChangeBlock(name: ts.Identifier, value: ts.Node | number, changed = false, overrideName = false): StatementNode {
    ...
    function getPropertyBlock(left: ts.PropertyAccessExpression, right: ts.Expression, tp: string): StatementNode | ExpressionNode {
        const info: CallInfo = pxtInfo(left).callInfo
    ...
    function getCallStatement(node: ts.CallExpression, asExpression: boolean): StatementNode | ExpressionNode {
        const info: CallInfo = pxtInfo(node).callInfo
        const attributes = attrs(info);
        ...
        args.forEach((arg, i) => {
            ...
            let shadowBlockInfo: pxtc.SymbolInfo;
            ...
            if (e.kind === SK.CallExpression) {
                ...
                const shimCall: CallInfo = pxtInfo(e).callInfo;
                const shimAttrs: pxtc.CommentAttrs = shimCall && attrs(shimCall);
            ...
            switch (e.kind) {
                ...
                case SK.ArrowFunction:
                    ...
                    if (m) {
                        ...
                        const addDraggableInput = (arg: pxtc.PropertyDesc, varName: string) => {
                    ...
                    break;
                case SK.PropertyAccessExpression:
                    const callInfo = pxtInfo(e).callInfo as CallInfo;
                    const aName = pxt.U.htmlEscape(param.definitionName);
                ...
                case SK.BinaryExpression:
                    if (param && param.shadowOptions && param.shadowOptions.toString) {
                        ...
                        if (be.operatorToken.kind === SK.PlusToken && isEmptyStringNode(be.left)) {
                            addInput(getValue(pxt.U.htmlEscape(param.definitionName), be.right, param.shadowBlockId || "text"));
                            break;
                        }
                    }
                    addInput(getValue(pxt.U.htmlEscape(param.definitionName), e, param.shadowBlockId, shadowMutation))
                    break;
                default:
                    let v: ValueNode;
                    const vName = pxt.U.htmlEscape(param.definitionName);
                    let defaultV = true;
                    ...
                    if (info.qName == "Math.random") {
                        ...
                        if (decompileLiterals(param) && param.fieldOptions['onParentBlock']) {
                            ...
                            if (field && decompileLiterals(field)) {
                                ...
                                if (param.shadowOptions) {
                                    fieldBlock.mutation = { "customfield": pxt.U.htmlEscape(JSON.stringify(param.shadowOptions)) };
                                }
                        ...
                        addField(getField(vName, pxt.U.htmlEscape(e.getText())));
                        return;
                    }
                    if (defaultV) {
    ...
    // function openCallExpressionBlockWithRestParameter(call: ts.CallExpression, info: CallInfo) {
    ...
    function getDestructuringMutation(callback: ts.ArrowFunction): pxt.Map<string> {
        ...
        if (bindings) {
            return {
                ...
                "renamemap": pxt.U.htmlEscape(JSON.stringify(bindings[1]))
            };
        }
    ...
    function codeBlock(statements: NodeArray<ts.Node>, next?: ts.Node[], topLevel = false, parent?: ts.Node, emitOnStart = false) {
     ...
     */
    function getCommentText(commentRanges: ts.CommentRange[], node: ts.Node, workspaceRefs?: string[]) {
        let text = ""
        let currentLine = ""
        ...
        function isTopLevelComment(n: ts.Node): boolean {
            const [parent,] = getParent(n);
    ...
    function addVariableDeclaration(node: VariableDeclaration): boolean {
        if (node.name.kind !== SK.Identifier) {
            error(node, pxt.U.lf("Variable declarations may not use binding patterns"))
            return false;
        }
        else if (!node.initializer) {
            error(node, pxt.U.lf("Variable declarations must have an initializer"))
            return false;
        }
        return true;
    }
...
function checkStatement(node: ts.Node, env: DecompilerEnv, asExpression = false, topLevel = false): string {
    ...
    return pxt.U.lf("Unsupported statement in block: {0}", SK[node.kind]);

    function checkForStatement(n: ts.ForStatement) {
        if (!n.initializer || !n.incrementor || !n.condition) {
            return pxt.U.lf("for loops must have an initializer, incrementor, and condition");
        }

        if (n.initializer.kind !== SK.VariableDeclarationList) {
            return pxt.U.lf("only variable declarations are permitted in for loop initializers");
        }

        const initializer = n.initializer as ts.VariableDeclarationList;

        if (!initializer.declarations) {
            return pxt.U.lf("for loop with out-of-scope variables not supported");
        }
        if (initializer.declarations.length != 1) {
            return pxt.U.lf("for loop with multiple variables not supported");
        }
        ...
        if (assignment.initializer.kind !== SK.NumericLiteral || (assignment.initializer as ts.LiteralExpression).text !== "0") {
            return pxt.U.lf("for loop initializers must be initialized to 0");
        }

        const indexVar = (assignment.name as ts.Identifier).text;
        if (!incrementorIsValid(indexVar)) {
            return pxt.U.lf("for loop incrementors may only increment the variable declared in the initializer");
        }

        if (n.condition.kind !== SK.BinaryExpression) {
            return pxt.U.lf("for loop conditionals must be binary comparison operations")
        }

        const condition = n.condition as ts.BinaryExpression
        if (condition.left.kind !== SK.Identifier || (condition.left as ts.Identifier).text !== indexVar) {
            return pxt.U.lf("left side of for loop conditional must be the variable declared in the initializer")
        }

        if (condition.operatorToken.kind !== SK.LessThanToken && condition.operatorToken.kind !== SK.LessThanEqualsToken) {
            return pxt.U.lf("for loop conditional operator must be either < or <=");
        }
    ...
    function checkForOfStatement(n: ts.ForOfStatement) {
        if (n.initializer.kind !== SK.VariableDeclarationList) {
            return pxt.U.lf("only variable declarations are permitted in for of loop initializers");
        }
    ...
    function checkBinaryExpression(n: ts.BinaryExpression, env: DecompilerEnv) {
        if (n.left.kind === SK.ElementAccessExpression) {
            if (n.operatorToken.kind !== SK.EqualsToken) {
                return pxt.U.lf("Element access expressions may only be assigned to using the equals operator");
            }
        }
        else if (n.left.kind === SK.PropertyAccessExpression) {
            if (n.operatorToken.kind !== SK.EqualsToken &&
                n.operatorToken.kind !== SK.PlusEqualsToken) {
                return pxt.U.lf("Property access expressions may only be assigned to using the = and += operators");
            } else {
            ...
            switch (n.operatorToken.kind) {
                ...
                case SK.MinusEqualsToken:
                    return undefined;
                default:
                    return pxt.U.lf("Unsupported operator token in statement {0}", SK[n.operatorToken.kind]);
            }
        }
        else {
            return pxt.U.lf("This expression cannot be assigned to")
        }
        return undefined;
    }
    ...
    function checkArrowFunction(n: ts.ArrowFunction, env: DecompilerEnv) {
        ...
        if (n.parameters.length) {
            ...
            if (parent && pxtInfo(parent).callInfo) {
                let callInfo: CallInfo = pxtInfo(parent).callInfo;
        ...
        if (fail) {
            return pxt.U.lf("Unsupported parameters in error function");
        }
        return undefined;
    }
    ...
    function checkIncrementorExpression(n: (ts.PrefixUnaryExpression | ts.PostfixUnaryExpression)) {
        if (n.operand.kind != SK.Identifier) {
            return pxt.U.lf("-- and ++ may only be used on an identifier")
        }

        if (n.operator !== SK.PlusPlusToken && n.operator !== SK.MinusMinusToken) {
            return pxt.U.lf("Only ++ and -- supported as prefix or postfix unary operators in a statement");
        }
    ...
    function checkVariableDeclaration(n: ts.VariableDeclaration, env: DecompilerEnv) {
        ...
        if (n.name.kind !== SK.Identifier) {
            check = pxt.U.lf("Variable declarations may not use binding patterns");
        }
        else if (!n.initializer) {
            check = pxt.U.lf("Variable declarations must have an initializer");
        }
    ...
    function checkCall(n: ts.CallExpression, env: DecompilerEnv, asExpression = false, topLevel = false) {
        const info: CallInfo = pxtInfo(n).callInfo;
        if (!info) {
            return pxt.U.lf("Function call not supported in the blocks");
        }
        ...
        if (!asExpression) {
            if (info.isExpression) {
                return pxt.U.lf("No output expressions as statements");
            }
        }
        ...
        if (attributes.blockId === pxtc.PAUSE_UNTIL_TYPE) {
            ...
            return pxt.U.lf("Predicates must be inline expressions that return a value");
        }

        const hasCallback = hasStatementInput(info, attributes);
        if (hasCallback && !attributes.handlerStatement && !topLevel) {
            return pxt.U.lf("Events must be top level");
        }
            ...
            if (!builtin) {
                if (n.expression.kind === SK.Identifier) {
                    ...
                    if (!env.declaredFunctions[funcName]) {
                        return pxt.U.lf("Call statements must have a valid declared function");
                    } else if (env.declaredFunctions[funcName].parameters.length !== info.args.length) {
                        return pxt.U.lf("Function calls in blocks must have the same number of arguments as the function definition");
                    } else {
                        return undefined;
                    }
                }
        ...
        if (attributes.imageLiteral) {
            ...
            if (info.args.length - totalDecompilableArgs > 1) {
                return pxt.U.lf("Function call has more arguments than are supported by its block");
            }

            let arg = n.arguments[0];
            if (arg.kind != SK.StringLiteral && arg.kind != SK.NoSubstitutionTemplateLiteral) {
                return pxt.U.lf("Only string literals supported for image literals")
            }
            const leds = ((arg as ts.StringLiteral).text || '').replace(/\s+/g, '');
            const nc = attributes.imageLiteral * 5;
            if (nc * 5 != leds.length) {
                return pxt.U.lf("Invalid image pattern");
            }
            return undefined;
        }
        ...
        if (api) {
            ...
            if (ns && ns.attributes.fixedInstances && !ns.attributes.decompileIndirectFixedInstances && info.args.length) {
                const callInfo: CallInfo = pxtInfo(info.args[0]).callInfo;
                if (!callInfo || !env.attrs(callInfo).fixedInstance) {
                    return pxt.U.lf("Fixed instance APIs can only be called directly from the fixed instance");
                }
            }
        }
        ...
        function checkArgument(arg: DecompileArgument) {
            ...
            if (paramInfo.isEnum) {
                if (checkEnumArgument(e)) {
                    ...
                    const callInfo: CallInfo = pxtInfo(e).callInfo;
                ...
                return pxt.U.lf("Enum arguments may only be literal property access expressions");
            }
                ...
                if (!dl) {
                    return pxt.U.lf("Field editor does not support literal arguments");
                }
            }
                ...
                if (!tagName) {
                    return pxt.U.lf("Tagged templates only supported in custom fields with param.fieldOptions.taggedTemplate set");
                }

                const tag = unwrapNode((e as ts.TaggedTemplateExpression).tag);

                if (tag.kind !== SK.Identifier) {
                    return pxt.U.lf("Tagged template literals must use an identifier as the tag");
                }

                const tagText = tag.getText();
                if (tagText.trim() != tagName.trim()) {
                    return pxt.U.lf("Function only supports template literals with tag '{0}'", tagName);
                }

                const template = (e as ts.TaggedTemplateExpression).template;

                if (template.kind !== SK.NoSubstitutionTemplateLiteral) {
                    return pxt.U.lf("Tagged template literals cannot have substitutions");
                }
            }
                ...
                if (ar.parameters.length) {
                    if (attributes.mutate === "objectdestructuring") {
                        ...
                        if (param.kind === SK.Parameter && param.name.kind !== SK.ObjectBindingPattern) {
                            return pxt.U.lf("Object destructuring mutation callbacks can only have destructuring patters as arguments");
                        }
                    }
                    else {
                        for (const param of ar.parameters) {
                            if (param.name.kind !== SK.Identifier) {
                                return pxt.U.lf("Only identifiers allowed as function arguments");
                            }
                        }
                    }
                }
                ...
                if (typeInfo.attributes.fixedInstances) {
                    ...
                    const callInfo = pxtInfo(e).callInfo;

                    if (callInfo && env.attrs(callInfo).fixedInstance) {
                        return undefined;
                    }

                    return pxt.U.lf("Arguments of a fixed instance type must be a reference to a fixed instance declaration");
                }
            }
    ...
    function checkFunctionDeclaration(n: ts.FunctionDeclaration, topLevel: boolean) {
        if (!topLevel) {
            return pxt.U.lf("Function declarations must be top level");
        }
            ...
            if (env.opts.allowedArgumentTypes) {
                for (const param of n.parameters) {
                    if (param.initializer || param.questionToken) {
                        return pxt.U.lf("Function parameters cannot be optional");
                    }

                    const type = param.type ? param.type.getText() : undefined;
                    if (!type) {
                        return pxt.U.lf("Function parameters must declare a type");
                    }

                    if (env.opts.allowedArgumentTypes.indexOf(normalizeType(type)) === -1) {
                        return pxt.U.lf("Only types that can be added in blocks can be used for function arguments");
                    }
                }
            }
        }
        ...
        let fail = false;
        forEachReturnStatement(n.body, stmt => {
            if (stmt.expression) {
                fail = true;
            }
        });
        ...
        if (fail) {
            return pxt.U.lf("Function with return value not supported in blocks")
        }
    ...
    function checkEnumDeclaration(n: ts.EnumDeclaration, topLevel: boolean) {
        if (!topLevel) return pxt.U.lf("Enum declarations must be top level");

        const name = n.name.text;
        const info = env.blocks.enumsByName[name];
        if (!info) return pxt.U.lf("Enum declarations in user code must have a block")
        ...
        if (fail) {
            return pxt.U.lf("Invalid initializer for enum member")
        }
    ...
    function checkNamespaceDeclaration(n: ts.NamespaceDeclaration) {
        if (!isModuleBlock(n.body)) {
            return pxt.U.lf("Namespaces cannot be nested.")
        }

        const kindInfo = env.blocks.kindsByName[n.name.text]
        if (!kindInfo) {
            return pxt.U.lf("Only namespaces with 'kind' blocks can be decompiled");
        }

        const fail = pxt.U.lf("Namespaces may only contain valid 'kind' exports");
...
function isEmptyStringNode(node: ts.Node) {
...
function isAutoDeclaration(decl: VariableDeclaration) {
    if (decl.initializer) {
        if (decl.initializer.kind === SyntaxKind.NullKeyword || decl.initializer.kind === SyntaxKind.FalseKeyword || isDefaultArray(decl.initializer)) {
            ...
            const callInfo: CallInfo = pxtInfo(decl.initializer).callInfo
...
function getObjectBindingProperties(callback: ts.ArrowFunction): [string[], pxt.Map<string>] {
    ...
    function checkName(name: ts.Node) {
        if (name && name.kind !== SK.Identifier) {
            // error(name, pxt.U.lf("Only identifiers may be used for variable names in object destructuring patterns"));
            return false;
        }
        return true;
    }
...
function checkExpression(n: ts.Node, env: DecompilerEnv): string {
    switch (n.kind) {
        ...
        case SK.Identifier:
            if (isUndefined(n)) {
                return pxt.U.lf("Undefined is not supported in blocks");
            } else if (isDeclaredElsewhere(n as Identifier)) {
                return pxt.U.lf("Variable is declared in another file");
            } else {
                return undefined;
            }
        case SK.BinaryExpression:
            const op1 = (n as BinaryExpression).operatorToken.getText();
            return ops[op1] ? undefined : pxt.U.lf("Could not find operator {0}", op1);
        case SK.PrefixUnaryExpression:
            const op2 = (n as PrefixUnaryExpression).operator;
            return op2 === SK.MinusToken || op2 === SK.PlusToken || op2 === SK.ExclamationToken ?
                undefined : pxt.U.lf("Unsupported prefix unary operator{0}", op2);
        case SK.PropertyAccessExpression:
    ...
    return pxt.U.lf("Unsupported syntax kind for output expression block: {0}", SK[n.kind]);

    function checkStringLiteral(n: ts.StringLiteral) {
        const literal = n.text;
        return validStringRegex.test(literal) ? undefined : pxt.U.lf("Only whitespace character allowed in string literals is space");
    }

    function checkPropertyAccessExpression(n: ts.PropertyAccessExpression, env: DecompilerEnv) {
        const callInfo: CallInfo = pxtInfo(n).callInfo;
        if (callInfo) {
            const attributes = env.attrs(callInfo);
            ...
            if (attributes.blockIdentity || attributes.blockId === "lists_length" || attributes.blockId === "text_length") {
                ...
                if (parent) {
                    const parentInfo: CallInfo = pxtInfo(parent).callInfo;
                ...
                if (fail) {
                    return pxt.U.lf("Enum value without a corresponding block");
                }
                else {
                    return undefined;
                }
        ...
        return pxt.U.lf("No call info found");
    }
}
...
function checkTaggedTemplateExpression(t: ts.TaggedTemplateExpression, env: DecompilerEnv) {
    ...
    if (!callInfo) {
        return pxt.U.lf("Invalid tagged template");
    }

    const attributes = env.attrs(callInfo);

    if (!attributes.blockIdentity) {
        return pxt.U.lf("Tagged template does not have blockIdentity set");
    }

    const api = env.blocks.apis.byQName[attributes.blockIdentity];

    if (!api) {
        return pxt.U.lf("Could not find blockIdentity for tagged template")
    }

    const comp = pxt.blocks.compileInfo(api);
    if (comp.parameters.length !== 1) {
        return pxt.U.lf("Tagged template functions must have 1 argument");
    }
...
function getParent(node: ts.Node): [ts.Node, ts.Node] {
    if (!node.parent) {
...
function hasStatementInput(info: CallInfo, attributes: pxtc.CommentAttrs): boolean {
...
function isFunctionExpression(node: ts.Node) {
    return node.kind === SK.ArrowFunction || node.kind === SK.FunctionExpression;
}

function paramList(info: CallInfo, blocksInfo: pxtc.BlocksInfo) {
...
function getEnumMembers(n: EnumDeclaration): [string, number][] {
    ...
    n.members.forEach(member => {
        pxt.U.assert(member.name.kind === SK.Identifier);
        ...
        if (member.initializer) {
            if (member.initializer.kind === SK.NumericLiteral) {
                ...
                pxt.U.assert(ex.left.kind === SK.NumericLiteral);
                pxt.U.assert((ex.left as NumericLiteral).text === "1");
                pxt.U.assert(ex.operatorToken.kind === SK.LessThanLessThanToken);
                pxt.U.assert(ex.right.kind === SK.NumericLiteral);
...
function isOutputExpression(expr: ts.Expression): boolean {
    switch (expr.kind) {
        ...
        case SK.CallExpression:
            const callInfo: CallInfo = pxtInfo(expr).callInfo
            pxt.U.assert(!!callInfo);

...
import { Type, Declaration, SyntaxKind, NamedDeclaration, TemplateHead, TemplateMiddle, TemplateTail, FunctionLikeDeclaration, SignatureDeclaration, ObjectLiteralElementLike, PropertySignature, ModuleDeclaration, ParameterDeclaration, PropertyDeclaration, Expression, TypeParameter, TypeReferenceNode, TypeFlags, LiteralExpression, MethodDeclaration, ObjectType, ClassDeclaration, TypeChecker, FunctionDeclaration, Identifier, ObjectFlags, TypeReference, SymbolFlags, IndexKind, SignatureKind, UnionType, InterfaceDeclaration, NodeArray, TypeParameterDeclaration, ClassLikeDeclaration, VariableDeclaration, BindingElement, TypeNode, Program, EmitResult, DiagnosticCategory, Statement, NodeFlags, ArrayLiteralExpression, AccessorDeclaration, TemplateExpression, TemplateSpan, JsxElement, JsxSelfClosingElement, JsxText, JsxExpression, QualifiedName, BindingPattern, ObjectLiteralExpression, PropertyAssignment, ShorthandPropertyAssignment, ComputedPropertyName, StringLiteral, PropertyAccessExpression, ElementAccessExpression, isClassDeclaration, ImportEqualsDeclaration, Signature, CallExpression, ModuleBlock, ConstructorDeclaration, NewExpression, ClassElement, TaggedTemplateExpression, TypeAssertion, AsExpression, ParenthesizedExpression, Block, DeleteExpression, TypeOfExpression, VoidExpression, AwaitExpression, PrefixUnaryExpression, PostfixUnaryExpression, BinaryExpression, EnumMember, NumericLiteral, VariableStatement, EnumDeclaration, ConditionalExpression, SpreadElement, YieldExpression, VariableDeclarationList, ExpressionStatement, IfStatement, DoStatement, WhileStatement, ForStatement, ForOfStatement, ForInStatement, BreakOrContinueStatement, LabeledStatement, ReturnStatement, WithStatement, SwitchStatement, CaseClause, CaseOrDefaultClause, ThrowStatement, TryStatement, CatchClause, ObjectBindingPattern, ArrayBindingPattern, ImportDeclaration, ExportDeclaration, ExportAssignment, SourceFile } from "../ext-typescript/lib/tsserverlibrary";

import { ir } from "./ir";

import { isPxtModulesFilename, CompileResult } from "./driver";

import { getFullName } from "./service";

import { hex, processorEmit, f4EncodeImg } from "./hexfile";

import { annotate } from "./annotate";

import { vmEmit } from "./backvm";

import { jsEmit } from "./backjs";

import { thumbCmpMap } from "./backthumb";
...
export class PxtNode {
    ...
    commentAttrs: pxtc.CommentAttrs;
    fullName: string;
...
function fitsTaggedInt(vn: number) {
    if (pxtcTarget.switches.boxDebug) return false
...
export function isInPxtModules(node: ts.Node) {
    if (node.pxt)
...
export function pxtInfo(n: ts.Node) {
    if (!n.pxt) {
...
export function getNodeId(n: ts.Node) {
    return pxtInfo(n).id
}

export function stringKind(n: ts.Node) {
    if (!n) return "<null>"
...
export function isStackMachine() {
    return pxtcTarget.isNative && pxtcTarget.nativeType == pxtc.NATIVE_TYPE_VM
}
...
export function isThumb() {
    return pxtcTarget.isNative && (pxtcTarget.nativeType == pxtc.NATIVE_TYPE_THUMB)
}
...
// everything else (except as indicated with pxtc.CommentAttrs), operates and returns regular ints
...
function getBitSize(decl: TypedDecl) {
    if (!decl || !decl.type) return pxtc.BitSize.None
    if (!(isNumberType(typeOf(decl)))) return pxtc.BitSize.None
    if (decl.type.kind != SK.TypeReference) return pxtc.BitSize.None
    switch ((decl.type as TypeReferenceNode).typeName.getText()) {
        case "int8": return pxtc.BitSize.Int8
        case "int16": return pxtc.BitSize.Int16
        case "int32": return pxtc.BitSize.Int32
        case "uint8": return pxtc.BitSize.UInt8
        case "uint16": return pxtc.BitSize.UInt16
        case "uint32": return pxtc.BitSize.UInt32
        default: return pxtc.BitSize.None
    }
}

export function sizeOfBitSize(b: pxtc.BitSize) {
    switch (b) {
        case pxtc.BitSize.None: return target.shortPointers ? 2 : 4
        case pxtc.BitSize.Int8: return 1
        case pxtc.BitSize.Int16: return 2
        case pxtc.BitSize.Int32: return 4
        case pxtc.BitSize.UInt8: return 1
        case pxtc.BitSize.UInt16: return 2
        case pxtc.BitSize.UInt32: return 4
        default: throw pxtc.oops()
    }
}

export function isBitSizeSigned(b: pxtc.BitSize) {
    switch (b) {
        case pxtc.BitSize.Int8:
        case pxtc.BitSize.Int16:
        case pxtc.BitSize.Int32:
            return true
        case pxtc.BitSize.UInt8:
        case pxtc.BitSize.UInt16:
        case pxtc.BitSize.UInt32:
            return false
        default: throw pxtc.oops()
    }
}
...
export function setCellProps(l: ir.Cell) {
    ...
    if (!isSyntheticThis(l.def)) {
        ...
        if (l.bitSize != pxtc.BitSize.None) {
    ...
    if (l.isLocal() && l.bitSize != pxtc.BitSize.None) {
        l.bitSize = pxtc.BitSize.None
...
export function getExplicitDefault(attrs: pxtc.CommentAttrs, name: string) {
...
function classFunctionPref(node: ts.Node) {
    if (!node) return null;
    switch (node.kind) {
...
function classFunctionKey(node: ts.Node) {
    return classFunctionPref(node) + getName(node)
}

function isClassFunction(node: ts.Node) {
    return classFunctionPref(node) != null
}

function getEnclosingMethod(node: ts.Node): MethodDeclaration {
    if (!node) return null;
...
function getEnclosingFunction(node0: ts.Node) {
    let node = node0
    let hadLoop = false
    while (true) {
        node = node.parent
...
function getRefTagToValidate(tp: string): number {
    switch (tp) {
        ...
        case "_Image": return pxtcTarget.imageRefTag || pxt.BuiltInType.RefImage
...
export let pxtcTarget: pxtc.CompileTarget;
export let compileOptions: pxtc.CompileOptions;
let lastSecondaryError: string
let lastSecondaryErrorCode = 0
let inCatchErrors = 0

export function getNodeFullName(checker: TypeChecker, node: ts.Node) {
...
export function parseCommentsOnSymbol(symbol: Symbol): pxtc.CommentAttrs {
    let cmts = ""
...
export function parseComments(node: ts.Node): pxtc.CommentAttrs {
...
export function getName(node: ts.Node & { name?: any; }) {
...
function typeCheckSubtoSup(sub: ts.Node | Type, sup: ts.Node | Type) {
...
function parentPrefix(node: ts.Node): string {
    if (!node)
        return ""
    switch (node.kind) {
        case SK.ModuleBlock:
...
export function compileBinary(
    ...
    let mainSrcFile = program.getSourceFiles().filter(f => pxt.U.endsWith(f.fileName, entryPoint))[0];
    ...
    if (diagnostics.getModificationCount() == 0) {
        ...
        pxt.U.assert(usedWorkList.length == 0)
        ...
        let pass1 = pxt.U.cpuUs()
        res.times["pass1"] = pass1 - pass0
        catchErrors(rootFunction, finalEmit)
        res.times["passFinal"] = pxt.U.cpuUs() - pass1
    }

    if (opts.ast) {
        let pre = pxt.U.cpuUs()
        annotate(program, entryPoint, target);
        res.times["passAnnotate"] = pxt.U.cpuUs() - pre
    }
    ...
    function diag(category: DiagnosticCategory, node: ts.Node, code: number, message: string, arg0?: any, arg1?: any, arg2?: any) {
    ...
    function warning(node: ts.Node, code: number, msg: string, arg0?: any, arg1?: any, arg2?: any) {
        diag(DiagnosticCategory.Warning, node, code, msg, arg0, arg1, arg2);
    }

    function error(node: ts.Node, code: number, msg: string, arg0?: any, arg1?: any, arg2?: any) {
        diag(DiagnosticCategory.Error, node, code, msg, arg0, arg1, arg2);
    }

    function unhandled(n: ts.Node, info?: string, code: number = 9202) {
    ...
    function getIfaceMemberId(name: string, markUsed = false) {
        return recordAction(bin => {
            if (markUsed) {
                if (!pxt.U.lookup(bin.explicitlyUsedIfaceMembers, name)) {
                    pxt.U.assert(!bin.finalPass)
                    bin.explicitlyUsedIfaceMembers[name] = true
                }
            }

            let v = pxt.U.lookup(bin.ifaceMemberMap, name)
            if (v != null) return v
            pxt.U.assert(!bin.finalPass)
    ...
    function getVTable(inf: ClassInfo) {
        pxt.U.assert(inf.isUsed, "inf.isUsed")
        if (inf.vtable)
            return inf.vtable
    ...
    function computeVtableInfo(info: ClassInfo) {
        for (let currMethod of info.allMethods()) {
            ...
            if (baseMethod) {
                ...
                pxt.U.assert(pinf.virtualParent == pinf, "pinf.virtualParent == pinf")
            }
        }
    }
    ...
    function getClassInfo(t: Type, decl: ClassDeclaration = null) {
        ...
        if (!pinfo.classInfo) {
            ...
            const prevFields = info.baseClassInfo
                ? pxt.U.toDictionary(info.baseClassInfo.allfields, f => getName(f)) : {}
            const prevMethod =
            ...
            for (let mem of decl.members) {
                if (mem.kind == SK.PropertyDeclaration) {
                    ...
                    if (prevMethod(key) || pxt.U.lookup(prevFields, key))
                    ...
                    const pfield = pxt.U.lookup(prevFields, key)
                    if (pfield) {
...
`)
    ...
    function emitFunLiteral(f: FunctionDeclaration) {
        ...
        if (info.location) {
            ...
            pxt.U.assert(!bin.finalPass || info.capturedVars.length == 0, "!bin.finalPass || info.capturedVars.length == 0")
    ...
    function emitThis(node: ts.Node) {
    ...
    function emitSuper(node: ts.Node) { }
    ...
    function emitObjectLiteral(node: ObjectLiteralExpression) {
        ...
        node.properties.forEach((p: PropertyAssignment | ShorthandPropertyAssignment) => {
            pxt.U.assert(!p.questionToken) // should be disallowed by TS grammar checker

            let keyName: string
            let init: ir.Expr
            if (p.kind == SK.ShorthandPropertyAssignment) {
                const sp = p as ShorthandPropertyAssignment
                pxt.U.assert(!sp.equalsToken && !sp.objectAssignmentInitializer) // disallowed by TS grammar checker
                keyName = p.name.text;
    ...
    function recordUsage(decl: Declaration) {
        if (!needsUsingInfo)
            return
        if (!currUsingContext) {
            pxt.U.oops("no using ctx for: " + getName(decl))
        } else {
            currUsingContext.usedNodes[nodeKey(decl)] = decl
        }
    }

    function getDeclCore(node: ts.Node): Declaration {
        if (!node) return null
        const pinfo = pxtInfo(node)
    ...
    function getDecl(node: ts.Node): Declaration {
    ...
    function getMask(args: Expression[]) {
        pxt.U.assert(args.length <= 8, "args.length <= 8")
        let m = 0
        args.forEach((a, i) => {
    ...
    function emitShim(decl: Declaration, node: ts.Node, args: Expression[]): ir.Expr {
        let attrs = parseComments(decl)
        ...
        if (nm.indexOf('(') >= 0) {
            ...
            if (parse) {
                if (args.length)
                    pxt.U.userError("no arguments expected")
                let litargs: ir.Expr[] = []
                ...
                if (strargs) {
                    for (let a of parse[2].split(/,/)) {
                        ...
                        if (isNaN(v)) {
                            ...
                            if (v == null)
                                pxt.U.userError("invalid argument: " + a + " in " + nm)
                        }
                        litargs.push(ir.numlit(v))
                    }
                    if (litargs.length > 4)
                        pxt.U.userError("too many args")
                }
                nm = parse[1]
                if (opts.target.isNative) {
        ...
        if (nm == "TD_ID" || nm === "ENUM_GET") {
            pxt.U.assert(args.length == 1, "args.length == 1")
    ...
    function addDefaultParametersAndTypeCheck(sig: Signature, args: Expression[], attrs: pxtc.CommentAttrs) {
        if (!sig) return;
    ...
    function emitCallCore(
        ...
        if (funcExpr.kind == SK.SuperKeyword) {
            ...
            pxt.U.assert(!bin.finalPass || !!baseCtor, "!bin.finalPass || !!baseCtor")
        ...
        if (isMethod) {
            ...
            if (needsVCall && !forceIfaceCall) {
                pxt.U.assert(!bin.finalPass || info.virtualIndex != null, "!bin.finalPass || info.virtualIndex != null")
    ...
    function mkProcCallCore(proc: ir.Procedure, args: ir.Expr[]) {
        pxt.U.assert(!bin.finalPass || !!proc)
        let data: ir.ProcId = {
            proc: proc,
            virtualIndex: null,
            ifaceIndex: null
    ...
    function mkProcCall(decl: ts.Declaration, args: ir.Expr[]) {
        ...
        pxt.U.assert(!!proc || !bin.finalPass, "!!proc || !bin.finalPass")
    ...
    function layOutGlobals() {
        ...
        const sz = (b: pxtc.BitSize) => b == pxtc.BitSize.None ? 10 : sizeOfBitSize(b)
        globals.sort((a, b) =>
        ...
        for (let g of globals) {
            ...
            while (currOff & (sz - 1))
                currOff++ // align
            if (!firstPointer && g.bitSize == pxtc.BitSize.None)
    ...
    function isIfaceMemberUsed(name: string) {
        return pxt.U.lookup(bin.explicitlyUsedIfaceMembers, name) != null
    }
    ...
    */
    function emitTaggedTemplateExpression(node: TaggedTemplateExpression): ir.Expr {
        ...
        function parseHexLiteral(node: ts.Node, s: string) {
            ...
            if (s == "" && thisJres) {
                ...
                if (fontMatch) {
                    if (!bin.finalPass) {
                        ...
                        s = pxt.U.toHex(pxt.U.stringToUint8Array(buf))
                    }
                } else if (!thisJres.dataEncoding || thisJres.dataEncoding == "base64") {
                    s = pxt.U.toHex(pxt.U.stringToUint8Array(ts.pxtc.decodeBase64(thisJres.data)))
    ...
    function markVariableDefinition(vi: VariableAddInfo) {
        if (bin.finalPass && vi.functionsToDefine) {
            pxt.U.pushRange(pendingFunctionDefinitions, vi.functionsToDefine)
        }
    }
    ...
    function emitFuncCore(node: FunctionLikeDeclaration) {
        ...
        if (bin.finalPass) {
            if (info.alreadyEmitted) {
                pxt.U.assert(info.usedBeforeDecl)
                return null
            }
        ...
        if (caps.length > 0) {
            pxt.U.assert(getEnclosingFunction(node) != null, "getEnclosingFunction(node) != null)")
        ...
        pxt.U.assert(!!lit == isExpression, "!!lit == isExpression")
        ...
        if (existing) {
            ...
            pxt.U.assert(!bin.finalPass, "!bin.finalPass")
            const pinfo = pxtInfo(node)
        ...
        if (node.parent.kind == SK.ClassDeclaration) {
            ...
            if (proc.classInfo)
                pxt.U.assert(proc.classInfo == classInfo, "proc.classInfo == classInfo")
            else
            ...
            if (node.kind == SK.Constructor) {
                ...
                if (classInfo.ctor)
                    pxt.U.assert(classInfo.ctor == proc, "classInfo.ctor == proc")
                else
                    classInfo.ctor = proc
        ...
        pxt.U.assert(!bin.finalPass || usedWorkList.length == 0, "!bin.finalPass || usedWorkList.length == 0")
    ...
    function enumValue(decl: EnumMember): string {
        ...
        if (/^0x[A-Fa-f\d]{2,8}$/.test(ev))
            return ev;
        pxt.U.userError("enumval only support number literals")
        return "0"
    }
    ...
    function lookupConfigConst(ctx: ts.Node, name: string) {
    ...
    function lookupConfigConstCore(ctx: ts.Node, name: string, mod: string) {
    ...
    function lookupDalConst(ctx: ts.Node, name: string) {
    ...
    function emitLit(v: number | boolean) {
        if (opts.target.isNative && !isStackMachine()) {
            ...
            if (numlit != null) return ir.numlit(numlit)
                ...
                throw pxt.U.oops("bad literal: " + v)
            }
        } else {
            return ir.numlit(v)
        }
    ...
    function rtcallMask(name: string, args: Expression[], attrs: pxtc.CommentAttrs, append: Expression[] = null) {
        let fmt: string[] = []
        let inf = hex.lookupFunc(name)

        if (isThumb()) {
            let inf2 = pxt.U.lookup(thumbFuns, name)
            if (inf2) {
                inf = inf2
                name = inf2.name
            }
        ...
        let args2 = args.map((a, i) => {
            ...
            if (!f) {
                throw pxt.U.userError("not enough args for " + name)
                ...
                //if (!isNumber)
                //    pxt.U.userError("argsFmt=...I... but argument not a number in " + name)
        ...
        if (opts.target.isNative) {
            ...
            if (f0 == "I")
                ...
                pxt.U.oops("double returns not yet supported") // take two words
                r = fromDouble(r)
            }
        }
        return r
    ...
    function simpleInstruction(node: ts.Node, k: SyntaxKind) {
        switch (k) {
    ...
    function emitVariableStatement(node: VariableStatement) {
        function addConfigEntry(ent: ConfigEntry) {
            let entry = pxt.U.lookup(configEntries, ent.name)
            if (!entry) {
                entry = ent
    ...
    function emitCondition(expr: Expression, inner: ir.Expr = null) {
        if (!inner && isThumb() && expr.kind == SK.BinaryExpression) {
            ...
            let mapped = pxt.U.lookup(thumbCmpMap, simpleInstruction(be, be.operatorToken.kind))
            if (mapped) {
    ...
    function getLabels(stmt: ts.Node) {
    ...
    function emitForOfStatement(node: ForOfStatement) {
        ...
        pxt.U.assert(!!iterVar || !bin.finalPass)
        proc.stackEmpty()
    ...
    function emitBreakOrContinueStatement(node: BreakOrContinueStatement) {
        ...
        function findOuter(parent: ts.Node): Statement {
    ...
    function emitSwitchStatement(node: SwitchStatement) {
        ...
        let lbls = node.caseBlock.clauses.map(cl => {
            ...
            if (cl.kind == SK.CaseClause) {
                ...
                // end in JS.
                pxt.U.assert(!defaultLabel, "!defaultLabel")
                defaultLabel = lbl
            } else {
                pxtc.oops()
            }
    ...
    function emitDebuggerStatement(node: ts.Node) {
        emitBrk(node)
    }
        ...
        if (node.kind === SK.BindingElement) {
            ...
            if (isGlobalVar(node)) {
                ...
                if (jrname) {
                    ...
                    let jr = pxt.U.lookup(opts.jres || {}, jrname)
                    if (!jr)
    ...
    function emitFieldAccess(node: ts.Node, objRef: ir.Expr, objType: Type, fieldName: string): [ir.Expr, Type] {
        const fieldSym = checker.getPropertyOfType(objType, fieldName);
        pxt.U.assert(!!fieldSym, "field sym")
    ...
    function catchErrors<T>(node: ts.Node, f: (node: ts.Node) => T): T {
    ...
    function emitExpr(node0: ts.Node, useCache: boolean = true): ir.Expr {
    ...
    function emitExprInner(node: ts.Node): ir.Expr {
    ...
    function emitTopLevel(node: Declaration): void {
        ...
        if (pinfo.usedNodes) {
            needsUsingInfo = false
            for (let node of pxt.U.values(pinfo.usedNodes))
                markUsed(node)
    ...
    function emit(node: ts.Node): void {
        catchErrors(node, emitNodeCore)
    }

    function emitNodeCore(node: ts.Node): void {
        switch (node.kind) {
            case SK.SourceFile:
    ...
    function emitExprCore(node: ts.Node): ir.Expr {
        switch (node.kind) {
            case SK.NullKeyword:
...
function doubleToBits(v: number) {
    ...
    a[0] = v
    return pxt.U.toHex(new Uint8Array(a.buffer))
}
...
export class Binary {
    ...
    getTitle() {
        const title = this.options.name || pxt.U.lf("Untitled")
        if (title.length >= 90)
    ...
    addProc(proc: ir.Procedure) {
        pxt.U.assert(!this.finalPass, "!this.finalPass")
    ...
    setPerfCounters(systemPerfCounters: string[]) {
        ...
        this.procs.forEach(p => {
            if (p.perfCounterName) {
                pxt.U.assert(target.switches.profile)

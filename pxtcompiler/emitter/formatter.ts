...
function tokenize(input: string) {
    ...
    // pxt.U.pxt.U.assert(tokens.map(t => t.text).join("") == input)
...
function delimitStmts(tokens: Token[], inStmtCtx: boolean, ctxToken: Token = null): Stmt[] {
    ...
    while (tokens[i].kind != TokenKind.EOF) {
        let stmtBeg = i
        skipToStmtEnd();
        pxt.U.pxt.U.assert(i > stmtBeg, `Error at ${tokens[i].text}`)
    ...
    function addStatement(tokens: Token[]) {
        ...
        if (inStmtCtx && res.length > 0) {
            ...
            if ((prevKind == SK.IfKeyword && thisKind == SK.ElseKeyword) ||
                ...
                pxt.U.pushRange(res[res.length - 1].tokens, tokens)
                return;
            }
        }
    ...
    function delimitIn(t: Token) {
        if (t.kind == TokenKind.Tree) {
            ...
            tree.children = pxt.U.concat(delimitStmts(tree.children, false, tree).map(s => s.tokens))
        }
    }
    ...
    function handleBlock() {
        pxt.U.pxt.U.assert(tokens[i].synKind == SK.OpenBraceToken)
        let tree = tokens[i] as TreeToken
        pxt.U.pxt.U.assert(tree.kind == TokenKind.Tree)
    ...
    function skipToStmtEnd() {
        while (true) {
            ...
            pxt.U.pxt.U.assert(!didBlock, "forgot continue/return after expectBlock")
            i++;
        }
    }
}

...
import { ModuleKind, Diagnostic, CompilerHost, createSourceFile, createProgram, LanguageService, Program } from "../ext-typescript/lib/typescript";
import { compileBinary, pxtcTarget } from "./emitter";
import { annotate } from "./annotate";
import { getApiInfo } from "./service";

export interface CompileResult extends pxtc.CompileResult {
    // Extend the CompileResult interface with ts specific fields
    ast?: ts.Program;
}

export function getTsCompilerOptions(opts: pxtc.CompileOptions) {
...
export function patchUpDiagnostics(diags: ReadonlyArray<Diagnostic>, ignoreFileResolutionErorrs = false) {
    ...
    return diags.map(d => {
        ...
        if (r.code == 1148)
            r.messageText = pxt.U.lf("all symbols in top-level scope are always exported; please use a namespace if you want to export only some")
        return r
    })
}

export function runConversions(opts: pxtc.CompileOptions) {
    let diags: pxtc.KsDiagnostic[] = []
    for (let pass of pxt.conversionPasses) {
        pxt.U.pushRange(diags, pass(opts))
    }
    return diags
}
...
export function storeGeneratedFiles(opts: pxtc.CompileOptions, res: CompileResult) {
...
export function runConversionsAndStoreResults(opts: pxtc.CompileOptions, res?: CompileResult) {
    const startTime = pxt.U.cpuUs()
    ...
    res.times["conversions"] = pxt.U.cpuUs() - startTime
...
function buildProgram(opts: pxtc.CompileOptions, res: CompileResult) {
    ...
    let tsFiles = opts.sourceFiles.filter(f => pxt.U.endsWith(f, ".ts"))
    return createProgram(tsFiles, options, host);
}

export function isPxtModulesFilename(filename: string) {
    return pxt.U.startsWith(filename, "pxt_modules/")
}

export function compile(opts: pxtc.CompileOptions, service?: LanguageService) {
    ...
    const entryPoint = opts.sourceFiles.filter(f => pxt.U.endsWith(f, ".ts")).pop().replace(/.*\//, "")
    ...
    // semantic errors.
    res.diagnostics = patchUpDiagnostics(program.getOptionsDiagnostics().concat(pxt.U.toArray(program.getGlobalDiagnostics())), opts.ignoreFileResolutionErrors);

    const semStart = pxt.U.cpuUs()
    ...
    if (res.diagnostics.length == 0) {
        if (opts.skipPxtModulesTSC) {
            ...
            res.diagnostics = patchUpDiagnostics(pxt.U.concatArrayLike(allDiag), opts.ignoreFileResolutionErrors)
        } else {
            res.diagnostics = patchUpDiagnostics(program.getSemanticDiagnostics(), opts.ignoreFileResolutionErrors);
        }
    }

    const emitStart = pxt.U.cpuUs()
    ...
    if (opts.ast || opts.forceEmit || res.diagnostics.length == 0) {
        ...
        res.times["compilebinary"] = pxt.U.cpuUs() - emitStart
    ...
    res.times["all"] = pxt.U.cpuUs() - startTime;
    pxt.tickEvent(`compile`, res.times);
    return res
}

export function decompile(program: Program, opts: pxtc.CompileOptions, fileName: string, includeGreyBlockMessages = false) {
    let file = program.getSourceFile(fileName);
    annotate(program, fileName, pxtcTarget || (pxt.appTarget && pxt.appTarget.compile));
    const apis = getApiInfo(program, opts.jres);
    const blocksInfo = pxtc.getBlocksInfo(apis, opts.bannedCategories);
    const decompileOpts: pxtc.decompiler.DecompileBlocksOptions = {
        snippetMode: false,
...
export function getTSProgram(opts: pxtc.CompileOptions, old?: ts.Program) {
    ...
    annotate(program, "main.ts", pxtcTarget || (pxt.appTarget && pxt.appTarget.compile));
    return program;
}

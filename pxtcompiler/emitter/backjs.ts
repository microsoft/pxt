import { ClassInfo, getName, Binary, FieldAccessInfo } from "./emitter"

import { ir } from "./ir"

import { nodeLocationInfo } from "./driver"
...
export function isBuiltinSimOp(name: string) {
    return !!pxt.U.lookup(jsOpMap, name.replace(/\./g, "::"))
}
...
function vtableToJs(info: ClassInfo) {
    pxt.U.assert(info.classNo !== undefined)
    pxt.U.assert(info.lastSubtypeNo !== undefined)
...
export function jsEmit(bin: Binary) {
    ...
    jssource += pxt.U.values(bin.codeHelpers).join("\n") + "\n"
...
`
    ...
    function emitJmp(jmp: ir.Stmt, isJmpNext: boolean) {
        if (jmp.lbl.lblNumUses == ir.lblNumUsesJmpNext || isJmpNext) {
            pxt.U.assert(jmp.jmpMode == ir.JmpMode.Always)
            if (jmp.expr)
                emitExpr(jmp.expr)
        ...
        pxt.U.assert(jmp.lbl.lblNumUses > 0)
    ...
    function emitExprInto(e: ir.Expr): string {
        switch (e.exprKind) {
            case EK.NumberLiteral:
                ...
                else throw pxtc.oops("invalid data: " + typeof e.data);
            case EK.PointerLiteral:
                if (e.ptrlabel()) {
    ...
    function emitRtCall(topExpr: ir.Expr) {
        ...
        else if (pxt.U.lookup(jsOpMap, name))
            text = args.length == 2 ? `(${args[0]} ${pxt.U.lookup(jsOpMap, name)} ${args[1]})` : `(${pxt.U.lookup(jsOpMap, name)} ${args[0]})`;
        else
    ...
    function emitProcCall(topExpr: ir.Expr) {
        ...
        if (callproc)
            ...
            bin.recordHelper(proc.usingCtx, id, () => {
                const locals = pxt.U.range(argLen).map(i => "arg" + i)
        ...
        if (procid.ifaceIndex != null) {
            ...
            pxt.U.assert(callproc == null)
            pxt.U.assert(procid.virtualIndex >= 0)
    ...
    function bitSizeConverter(b: pxtc.BitSize) {
        switch (b) {
            case pxtc.BitSize.None: return ""
            case pxtc.BitSize.Int8: return "pxtrt.toInt8"
            case pxtc.BitSize.Int16: return "pxtrt.toInt16"
            case pxtc.BitSize.Int32: return "pxtrt.toInt32"
            case pxtc.BitSize.UInt8: return "pxtrt.toUInt8"
            case pxtc.BitSize.UInt16: return "pxtrt.toUInt16"
            case pxtc.BitSize.UInt32: return "pxtrt.toUInt32"
            default: throw pxtc.oops()
        }
    }

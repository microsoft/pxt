import { pxtcTarget } from "./emitter";
...
export class ThumbProcessor extends pxtc.assembler.AbstractProcessor {
    ...
    public toFnPtr(v: number, baseOff: number, lbl: string) {
        if (pxtcTarget.runtimeIsARM && /::/.test(lbl))
            return (v + baseOff) & ~1;
        return (v + baseOff) | 1;
    }
    ...
    public is32bit(i: pxtc.assembler.Instruction) {
        return i.name == "bl" || i.name == "bb";
    }

    public postProcessAbsAddress(f: pxtc.assembler.File, v: number) {
    ...
    public expandLdlit(f: pxtc.assembler.File): void {
        let nextGoodSpot: pxtc.assembler.Line
        let needsJumpOver = false
        let outlines: pxtc.assembler.Line[] = []
    ...
    public peephole(ln: pxtc.assembler.Line, lnNext: pxtc.assembler.Line, lnNext2: pxtc.assembler.Line) {
        ...
        function fits(enc: pxtc.assembler.Encoder, ln: pxtc.assembler.Line) {
            return (
...
function singleReg(ln: pxtc.assembler.Line) {
    pxt.U.assert(ln.getOp() == "push" || ln.getOp() == "pop")
    let k = 0;
    let ret = -1;
    let v = ln.numArgs[0]
    while (v > 0) {

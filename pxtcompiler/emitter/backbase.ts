...
import { Binary, sizeOfBitSize, FieldAccessInfo, ClassInfo, taggedTrue, taggedFalse, taggedNull, isThumb, pxtcTarget } from "./emitter"
import { asmline, firstMethodOffset, vtLookups, hex } from "./hexfile"
...
export abstract class AssemblerSnippets {
    ...
    string_literal(lbl: string, strLit: string) {
        ...
        let utfLit = pxtcTarget.utf8 ? pxt.U.toUTF8(strLit, true) : strLit
        if (utfLit !== strLit) {
            if (strLit.length > SKIP_INCR) {
                ...
                for (let i = 0; i + SKIP_INCR <= strLit.length; i += SKIP_INCR) {
                    off += pxt.U.toUTF8(strLit.slice(i, i + SKIP_INCR), true).length
                    skipList.push(off)
                }
                return `
.balign 4
...
export class ProctoAssembler {
    ...
    private calls: pxtc.ProcCallInfo[] = []
    ...
    private stackAlignmentNeeded(offset = 0) {
        if (!pxtcTarget.stackAlign) return 0
        let npush = pxtcTarget.stackAlign - ((this.stackSize() + offset) & (pxtcTarget.stackAlign - 1))
        if (npush == pxtcTarget.stackAlign) return 0
        else return npush
    }
...
`)
        ...
        if (this.proc.classInfo && this.proc.info.thisParameter
            && !pxtcTarget.switches.skipClassCheck
            && !pxtcTarget.switches.noThisCheckOpt) {
...
`)
        ...
        this.proc.fillDebugInfo = th => {
            ...
            this.proc.debugInfo = {
                ...
                codeStartLoc: pxt.U.lookup(labels, locLabel),
                codeEndLoc: pxt.U.lookup(labels, endLabel),
                bkptLoc: pxt.U.lookup(labels, bkptLabel),
                localsMark: pxt.U.lookup(th.stackAtLabel, locLabel),
                idx: this.proc.seqNo,
                calls: this.calls
            }

            for (let ci of this.calls) {
                ci.addr = pxt.U.lookup(labels, ci.callLabel)
                ci.stack = pxt.U.lookup(th.stackAtLabel, ci.callLabel)
            ...
            for (let i = 0; i < this.proc.body.length; ++i) {
                ...
                if (bi) {
                    let off = pxt.U.lookup(th.stackAtLabel, `__brkp_${bi.id}`)
                    if (off !== this.proc.debugInfo.localsMark) {
                        console.log(bi)
                        console.log(th.stackAtLabel)
                        pxt.U.oops(`offset doesn't match: ${off} != ${this.proc.debugInfo.localsMark}`)
                    }
                }
            }
        }
        ...
        pxt.U.assert(0 <= numlocals && numlocals < 127);
        if (numlocals > 0)
    ...
    private terminate(expr: ir.Expr) {
        pxt.U.assert(expr.exprKind == ir.EK.SharedRef)
        let arg = expr.args[0]
        ...
        pxt.U.assert(this.exprStack[0] === arg, "term at top")
    ...
    private validateJmpStack(lbl: ir.Stmt, off = 0) {
        ...
        if (lbl.lblStackSize == null) {
            ...
            if (lbl.lblStackSize != currSize) {
                ...
                pxt.U.oops("stack misaligned at: " + lbl.lblName)
            }
        }
    }
    ...
    private emitJmp(jmp: ir.Stmt) {
        ...
        if (jmp.jmpMode == ir.JmpMode.Always) {
            ...
            if (jmp.expr.exprKind == ir.EK.RuntimeCall &&
                (jmp.expr.data === "thumb::subs" || pxt.U.startsWith(jmp.expr.data, "_cmp_"))) {
                // no cmp required
            } else {
    ...
    private bitSizeInfo(b: pxtc.BitSize) {
        let inf: BitSizeInfo = {
        ...
        if (b == pxtc.BitSize.Int8 || b == pxtc.BitSize.Int16) {
    ...
    private emitFieldAccess(e: ir.Expr, store = false) {
        ...
        if (info.needsCheck && !pxtcTarget.switches.skipClassCheck) {
    ...
    private emitClassCall(procid: ir.ProcId) {
        ...
        this.emitLabelledHelper("classCall_" + info.id + suff, () => {
            ...
            this.loadVTable()
            if (!pxtcTarget.switches.skipClassCheck && !procid.isThis)
    ...
    private emitIfaceCall(procid: ir.ProcId, numargs: number, getset = "") {
        pxt.U.assert(procid.ifaceIndex > 0)
    ...
    private clearArgs(nonRefs: ir.Expr[], refs: ir.Expr[]) {
        ...
        for (let r of allArgs) {
            if (r.currUses != 0 || r.totalUses != 1) {
                ...
                pxtc.oops(`wrong uses: ${r.currUses} ${r.totalUses}`)
            }
            r.currUses = 1
        }
        this.clearStack()
    ...
    private emitRtCall(topExpr: ir.Expr, genCall: () => void = null) {
        ...
        pxt.U.assert(allArgs.length <= 4)

        let seenUpdate = false
        for (let a of pxt.U.reversed(allArgs)) {
            if (a.expr.isPure()) {
        ...
        if (complexArgs.length == 1 && !c0.conv && !c0.isRef) {
            ...
            if (convArgs.length) {
                const conv = this.redirectOutput(() => {
                    let off = 0
                    if (!pxtcTarget.switches.inlineConversions) {
                    ...
                    for (let a of convArgs) {
                        if (isThumb() && a.conv.method == "pxt::toInt") {
                            ...
                            let idx = pxtcTarget.switches.inlineConversions ? a.expr.getId() : off
                            ...
                            this.write(this.loadFromExprStack("r0", a.expr, off))
                            if (a.conv.refTag) {
                                if (!pxtcTarget.switches.skipClassCheck)
                    ...
                    for (let a of pxt.U.reversed(convArgs)) {
                        off--
    ...
    private alignedCall(name: string, cmt = "", off = 0, saveStack = false) {
        if (pxt.U.startsWith(name, "_cmp_") || pxt.U.startsWith(name, "_pxt_"))
            saveStack = false
    ...
    private emitArrayMethod(op: string, isBuffer: boolean) {
        ...
        const ldrSize = pxtcTarget.stackAlign || isBuffer ? "ldr" : "ldrh"
    ...
    private emitLambdaTrampoline() {
        let r3 = pxtcTarget.stackAlign ? "r3," : ""
        this.write(`
            .section code
    ...
    private emitProcCall(topExpr: ir.Expr) {
        ...
        let seenUpdate = false
        for (let c of pxt.U.reversed(topExpr.args)) {
            if (c.isPure()) {
        ...
        let argsToPush = pxt.U.reversed(topExpr.args)
        ...
        if (isLambda) {
            ...
            if (custom) {
                ...
                pxt.U.assert(isSet == (topExpr.args.length == 2))
                pxt.U.assert(!isSet == (topExpr.args.length == 1))
    ...
    private emitCallRaw(name: string) {
        ...
        pxt.U.assert(!!inf, "unimplemented raw function: " + name)

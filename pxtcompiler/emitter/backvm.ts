import { ClassInfo, Binary, taggedSpecial, FieldAccessInfo } from "./emitter"

import { computeHashMultiplier, hex, assemble } from "./hexfile"

import { hexLiteralAsm } from "./backbase"
...
function vtableToVM(info: ClassInfo, opts: CompileOptions, bin: Binary) {
    ...
    */
    ...
    const mapping = pxt.U.toArray(ifaceInfo.mapping)
...
`
    ...
    s += "  .short " + pxt.U.toArray(mapping).map((e, i) => offsets[e + ""] || zeroOffset).join(", ") + "\n"
    s += descs
...
_start_${name}:
    ...
    const now = new Date()
    let encodedName = pxt.U.toUTF8(opts.name, true)
...
`
    ...
    while (s) {
        ...
        if (pref.length & 1)
            pref += "\x00"
        opcm += ".hex " + pxt.U.toHex(pxt.U.stringToUint8Array(pref)) + "\n"
    }
...
function irToVM(ctx: EmitCtx, bin: Binary, proc: ir.Procedure): string {
    ...
    function cellref(cell: ir.Cell) {
        if (cell.isGlobal()) {
            ...
            pxt.U.assert(!final || idx < numLoc, "cell#" + idx)
            pxt.U.assert(idx >= 0, "cell#" + idx)
    ...
    function callRT(name: string) {
        ...
        if (!inf) pxt.U.oops("missing function: " + name)
        ...
        switch (e.exprKind) {
            case EK.NumberLiteral:
                ...
                if (tagged != null)
                    ...
                    let key = n0 + "," + n1
                    let id = pxt.U.lookup(ctx.dbls, key)
                    if (id == null) {
            ...
            case EK.CellRef:
                ...
                if (cell.isGlobal()) {
                    // TODO
                    if (cell.bitSize == pxtc.BitSize.Int8) {
                        write(`sgnext`)
                    } else if (cell.bitSize == pxtc.BitSize.UInt8) {
                        write(`clrhi`)
                    }
                }
                return
    ...
    function emitRtCall(topExpr: ir.Expr) {
        ...
        name = pxt.U.lookup(vmCallMap, name) || name

        clearStack()

        let spec = pxt.U.lookup(vmSpecOpcodes, name)
    ...
    function emitProcCall(topExpr: ir.Expr) {
        ...
        if (lambdaArg) {
            ...
            if (/Set/.test(calledProcId.mapMethod)) {
                write(`callset ${idx}`)
                pxt.U.assert(nargs == 2)
            }
            else if (/Get/.test(calledProcId.mapMethod)) {
                write(`callget ${idx}`)
                pxt.U.assert(nargs == 1)
            } else
    ...
    function emitStore(trg: ir.Expr, src: ir.Expr) {
        switch (trg.exprKind) {
            case EK.CellRef:
                ...
                if (cell.isGlobal() &&
                    (cell.bitSize == pxtc.BitSize.Int8 || cell.bitSize == pxtc.BitSize.UInt8)) {

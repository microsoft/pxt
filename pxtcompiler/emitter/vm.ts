...
export class VmInstruction extends pxtc.assembler.Instruction {
    constructor(ei: pxtc.assembler.AbstractProcessor, format: string, opcode: number) {
        super(ei, format, opcode, opcode, false)
    }

    emit(ln: pxtc.assembler.Line): pxtc.assembler.EmitResult {
        let tokens = ln.words;
...
export class VmProcessor extends pxtc.assembler.AbstractProcessor {
    ...
    public postProcessRelAddress(f: pxtc.assembler.File, v: number): number {
        return v;
    }

    public postProcessAbsAddress(f: pxtc.assembler.File, v: number): number {
        return v;
    }

    public getAddressFromLabel(f: pxtc.assembler.File, i: pxtc.assembler.Instruction, s: string, wordAligned = false): number {

...
Scheduling and concurrency is shared by both high and low level languages, and therefore handled uniformly by \CO. This means that when a foreign function call from blocks is mapped to C++, blocking semantics are handled directly in that mapping, \emph{translating the semantics of the blocks transparently}. The ``on light bright'' block, roughly maps to Figure~\ref{fig:messageBus}.

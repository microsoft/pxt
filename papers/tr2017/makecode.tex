...
programming using Blocky, to text-based programming in JavaScript (actually, Static TypeScript) using Monaco, while leveraging C++ on the backend for efficient use of MCU resources.
...
The app is built from a \MC ``target'' (\emph{\dbhref{https://github.com/microsoft/pxt-adafruit}{pxt-adafruit}})
which parameterizes the \MC framework (\emph{\dbhref{https://github.com/microsoft/pxt}{pxt}}).
...
The \MC compiler supports the STS language subset described in Section~\ref{sec:sts},
...
point and the \texttt{null} and \texttt{undefined} values are represented by the default integer value of zero.

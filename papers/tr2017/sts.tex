...
\end{itemize}
\item $S$ and $T$ are constructor function types and XYZ;
\item $S$ and $T$ are array types and XYZ;
\end{itemize}

Given types $S$ and $T$, checkMemb($S$,$T$) holds if for each member $M$ in $T$, one of the following is true:
\begin{itemize}
\item $M$ is a property and $S$ has a property $N$ where all of the following are true:
\begin{itemize}
\item $M$ and $N$ have the same name;
\item the type of $N$ is a subtype of the type of $M$;
\item $M$ and $N$ are both public, or $M$ and $N$ are both private (protected) and originate in the same declaration, or $M$ is protected and $N$ is declared in a class derived from the class in which M is declared;
\end{itemize}
\item $M$ is a string index signature of type $U$, and $S$ has an string index signature of a type that is a subtype of $U$.
\end{itemize}


\subsection{Compilation}

% Classic vtable for classes (prefix); classic interface table 

...
In STS, $S$ is a subtype of a type $T$ if one of the following is true:\footnote{
...
\begin{itemize}
\item $T$ has a numeric index signature with element type $U$, and $S$ has a numeric index signature with element type $V$ such that $V$ is a subtype of $U$;
\item checkMemb($S$,$T$) holds.
\end{itemize}
\end{itemize}

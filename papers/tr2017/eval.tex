...
To gauge the cost of different asynchronous operations in \CON, we tested three commonly used code paths using the CPX: (1) creating a fiber; (2) handling a non-blocking procedure call; and (3) handling a blocking procedure call. Table~\ref{table:time-ram-consumption} presents our results. These code paths are used to invoke handlers for events with corresponding listeners, central to \CO and \MC.

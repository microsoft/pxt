...
In our running example, a listen call to the MessageBus with a component ID of the light sensor and a threshold event is the underlying mechanism to implement the runtime function
\emph{onLightConditionChanged} from Figure~\ref{fig:example}, 
as illustrated by the equivalent C++ code snippet in Figure~\ref{fig:messageBus}.
...
This approach brings the benefits of abstraction and reusability to \CO, but without losing the hardware specific benefits seen in flat abstraction models where every MCU is made to look the same, even though their capabilities are different. This can be seen in the Arduino and mbed APIs, for example.

Finally, we group together the components of a physical device to form a \emph{device model}. This is a singelton C++ class that, through composition of device driver components, provides a configured representation of the capabilities of a device. Such a model provides for an elegant OO API for programming a device and also a static representation that forms an ideal target for the \MC linker to bind high level STS interfaces to low level optimised code. An example device model for the Adafruit CPX is shown in Figure~\ref{fig:codalDeviceModel} for reference.
